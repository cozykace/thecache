...
(function () {
  ...
  var W_INTERNAL = ["money.cloud", "money.cloudKey", "money.cloudPaused", "money.deviceId", "money.__lmeta", "money.dockMobile", "money.zoom", "money.gutter", "money.sidebar", "money.sidebarWidth", "money.statsScroll", "money.icons.collapsed", "money.balExpanded", "money.settings", "money.connect", "money.wiki", "money.timerRun", "money.deckDay", "money.dms", "money.simplefin", "money.sessionRun", "money.justReset", "money.waterfx", "money.brandkit", "money.brandkitApply", "money.deckRev"];   // sessionRun/justReset/waterfx per-device; deckDay siloed; dms + simplefin (browser bank bearer secret) never ride the vault; brandkit = device MIRROR of the COMPANY cache's kit (never the founder's own vault); brandkitApply = per-device "apply live" toggle; deckRev RETIRED — MUST match app.js DEVICE_LOCAL_KEYS or it'd sync as a generic key and churn
  ...
  function serve(url, method, body) {
    ...
    if (url.indexOf("/api/") !== -1) {
      ...
      // the paycheck-runway anchor + every income rhythm (the calendar's income layer),
      ...
      if (M === "GET" && key === "runway" && window.CacheMoney) {
        try {
          ...
          var cx3 = _moneyCtx();
          var rh3 = window.CacheMoney.incomeRhythms(all3, cx3.incomeOverrides, cx3.overrides, cx3.remap);
          return J({ next_deposit: window.CacheMoney.nextDepositOf(rh3), rhythms: rh3 });
        } catch (e) {}
      }
