...
for t in merge_test authored_test livelock_test profile_test profilecard_test timer_test deck_test things_test forms_test logderive_test crypto_interop_test keybox_test vaultmigrate_test social_test routine_test cal_test deckday_test escape_test notifs_test bugcredit_test resetpw_test webmoney_test brandkit_test viz_test; do
  run "$t" node "tests/$t.js"
done

echo "── Python (backend merge engine) ──"
for t in map_test roundtrip_test maptie_test restore_test rev_test catmeta_test tomb_test deck_parity checkinlog_test simplefin_test webmoney_parity burn_test catkey_test debt_test manual_test annual_test runway_test roles_test; do
