...
- [ ] **Income forecast → stacked streams view** (IN PROGRESS) — stacked income-by-source over time: real history (per-month, per-source from the ledger) → flat slider projection, with a "needed" line, "now" divider, per-source legend toggles, and a mode toggle to the existing cushion/goal view. Backend `/api/income-monthly` shipped 2026-06-26; chart frontend + Toggl-effort overlay next. Requested 2026-06-26.
