...
**API** (`server.py`, 127.0.0.1) — GET: `ping` `connect-status` `summary` `categories` `recurring` `transfers` `deposits` `merchants` `other-merchants` `averages` `work` `income-monthly` `work-monthly` `integrity` `issues` `subs` `income-links` `match-count` `bugs` `update-check`. POST: `categorize` `income` `category` `sync` `import` `import-data` `merge-maps` (cross-device newest-per-key merge of the FIVE user-authored files — categories/income/subs/income_links + **catmeta** (renames newest-per-key, custom categories unioned+SORTED — an unsorted union list would livelock the authored-hash witness) — via the vault's `filesMeta` sidecar; recomputes if it changed anything. `import_data` routes these five through the same merge instead of replacing them) `bug` `bug-status` `subs` `income-links` `delete-txn` `connect` `update` `restart`.
