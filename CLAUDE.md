...
- *cloud*: `cloud` (account/session state) `cloudKey` (the device's vault data key — never rides in the encrypted bundle; excluded by snapshotLocal/restoreLocal) `cloudPaused` (local-only toggle)
