...
- **Public demo page** (`docs/demo.html`, served at `https://cozykace.github.io/thecache/demo.html`, embedded in Squarespace via a Code Block). A self-contained mock of the dashboard using **play numbers only** — never real balances/merchants/account data. Same publish path as the roadmap (push to `main` → live). Keep it on-brand (Cache gold `#FFD409`, warm-dark) and keep the "every number is made up" disclaimer.
